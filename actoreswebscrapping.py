...
for gen in GENERO:
    ...
    di1=join(di,gen+'\\')
    for k in range(0,min(actores_totales,10000),50):
        url_imdb = "https://www.imdb.com/search/name?gender="+gen+"&start="+str(1+k)+"&ref_=rlm"
        response = http.request('GET',url_imdb)
        soup = BeautifulSoup(response.data)
        ...
        for k in range(A.shape[0]):
            ...
            url_gettyimages="https://www.gettyimages.com/photos/{n[0]}?compositions=headshot&family=editorial&numberofpeople=one&phrase={n[1]}&recency=last12months&sort=newest".format(n=changeFormat(A[1].iloc[k]))
            ...
            if n>0:
                ...
                dic={'Nombre':[name],"genero":[gen],"Nacimiento":[bday],"Edad":[edad]}
                if url_photo.find("nopicture")<0:
